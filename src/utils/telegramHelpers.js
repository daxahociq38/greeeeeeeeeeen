...
function safe(fn) {
  try { if (fn.isAvailable?.()) fn(); else fn(); } catch { /* ignore */ }
}
...
export function initTelegramApp(bgColor = '#FFFFFF', headerColor = '#FFFFFF') {
  ...
  try { init(); } catch { /* outside Telegram */ }
  ...
  if (setMiniAppBackgroundColor.isAvailable?.()) {
    try { setMiniAppBackgroundColor(bgColor); } catch {}
  }
  if (setMiniAppHeaderColor.isAvailable?.()) {
    try { setMiniAppHeaderColor(headerColor); } catch {}
  }
}
...
export function setupBackButton(onBack) {
  ...
  if (backButton.onClick.isAvailable?.()) {
    try { off = backButton.onClick(onBack); } catch {}
  }
...
export function getUserId() {
  return window.Telegram?.WebApp?.initDataUnsafe?.user?.id || 'browser_user';
}
